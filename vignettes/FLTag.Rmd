...
There are some strange observations and missing values which we can clean out as follows.
...
Usually the first thing we should do prior to any data analysis is to plot the data. Here we plot the location of the releases using `ggplot2` and add a layer for Large Marine Ecosystems and the FAD moratorium 2016 which are built-in to `FLTag`.
...
Merging the tag release data to the tag recovery table is then essential for estimating important parameters such as growth, mortality and migration. In the following the data are organised in a series of pairs of status release and recovery.

```{r adding on release data to recoveries table and ploting length-frequency densities by species ,echo=TRUE}
...
An example of plotting vectors of movement arrows using ggplot2. Note the substantial migrations of skipjack from the Azores.

```{r skj vectors,echo=TRUE,eval=FALSE}
